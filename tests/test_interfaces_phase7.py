...
async def test_release_api_plans_verified_updates(tmp_path: Path) -> None:
    app = create_app(AppConfig(tmp_path / "data", tmp_path / "workspace", tmp_path / "logs", tmp_path / "data" / "state.db"))
    manifest = {
        "version": "0.1.28", "channel": "stable", "release_url": "https://example.invalid/releases/v0.1.28",
...
async def test_startup_marks_matching_pending_update_known_good(tmp_path: Path) -> None:
    ...
    app.state.database.execute("INSERT INTO update_history(id, version, action, status, metadata_json) VALUES ('pending', '0.1.27', 'native_update', 'installed_pending_health', ?)", (json.dumps({"backup": {"verified": True, "schema_version": 11}}),))
