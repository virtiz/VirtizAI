...
class DelegationPolicyEngine:
    ...
    async def _classify(self, content: str) -> DelegationDecision | None:
        ...
        try:
            ...
            tool = {"type":"function","function":{"name":"delegation_decision","parameters":{"type":"object","additionalProperties":False,"required":["decision","role_id","confidence","reason_code"],"properties":{"decision":{"type":"string","enum":["direct","delegate"]},"role_id":{"type":["string","null"]},"confidence":{"type":"number","minimum":0,"maximum":1},"reason_code":{"type":"string","enum":["coding_request","ordinary_conversation","technical_explanation","ambiguous"]}}}}}
            response = await self.providers.chat(candidates[0].provider_id, candidates[0].model_name, [{"role":"system","content":"Use delegation_decision exactly once. Delegate only clear repository coding work; explanations and ambiguity are direct."},{"role":"user","content":content}], max_tokens=96, tools=[tool])
            ...
            if choice not in {"direct","delegate"} or not isinstance(confidence,(int,float)) or not 0 <= confidence <= 1 or reason not in {"coding_request","ordinary_conversation","technical_explanation","ambiguous"}: return None
            if choice == "direct": return DelegationDecision("direct", None, float(confidence), reason, "model", "") if role is None else None
            if role != "role-coding" or confidence < self.high_confidence: return DelegationDecision("direct", None, float(confidence), "classifier_below_threshold", "model", "")
